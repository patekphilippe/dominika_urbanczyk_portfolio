...
const contentImgSelector = `.${styles.contentImg}`;
const contentImgInnerSelector = `.${styles.contentImgInner}`;
...
function getEventTarget(container: HTMLDivElement): HTMLElement {
  const target = container.closest<HTMLElement>("[data-imagetrail-target]");
  return target ?? container.parentElement ?? container;
}
...
class ImageItem {
  ...
  constructor(DOM_el: HTMLDivElement) {
    this.DOM.el = DOM_el;
    this.DOM.inner = this.DOM.el.querySelector(contentImgInnerSelector);
    this.getRect();
    this.initEvents();
  }
...
class ImageTrailVariant1 {
  ...
  constructor(container: HTMLDivElement) {
    ...
    this.images = [...container.querySelectorAll(contentImgSelector)].map(
    ...
    const eventTarget = getEventTarget(container);

    const handlePointerMove = (ev: MouseEvent | TouchEvent) => {
      const rect = this.container.getBoundingClientRect();
      this.mousePos = getLocalPointerPos(ev, rect);
    };
    eventTarget.addEventListener("mousemove", handlePointerMove);
    ...
    const initRender = (ev: MouseEvent | TouchEvent) => {
      ...
      eventTarget.removeEventListener("mousemove", initRender as EventListener);
      eventTarget.removeEventListener("touchmove", initRender as EventListener);
    };
    eventTarget.addEventListener("mousemove", initRender as EventListener);
    eventTarget.addEventListener("touchmove", initRender as EventListener);
  }
...
export default function ImageTrail({
  ...
  return (
    <div className={styles.content} ref={containerRef}>
      {items.map((url, i) => (
        <div className={styles.contentImg} key={i}>
          <div
